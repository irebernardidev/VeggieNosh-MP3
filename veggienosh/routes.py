...
def home():
    ...
    featured_recipes = list(recipes_coll.aggregate([{"$sample": {"size": 4}}]))

    # Render the home page with featured recipes
    return render_template(
        "home.html", featured_recipes=featured_recipes, title="VeggieNosh Home"
    )
...
def single_recipe_info(recipe_id):
    ...
    author = users_coll.find_one(
        {"_id": ObjectId(selected_recipe.get("author"))})[
        "username"
    ]
    return render_template(
        "single_recipe_info.html",
...
def my_recipes(username):
    ...
    my_username = users_coll.find_one(
    ...
    # Sort recipes by ID in ascending order for
    # the current page and limit the results per page
    recipes_displayed = (
        my_recipes_cursor.sort("_id", pymongo.ASCENDING)
        .skip((current_page - 1) * limit_per_page)
        .limit(limit_per_page)
    )
    ...
    return render_template(
        "my_recipes.html",
        my_recipes=list(recipes_displayed),  # Convert cursor to a list
...
def insert_recipe():
    ...
    if request.method == "POST":
        ...
        # and redirect to the new recipe's page
        flash("Your recipe was successfully added!")
        return redirect(
            url_for("single_recipe_info",
...
def update_recipe(recipe_id):
    ...
    if request.method == "POST":
        ...
        # After updating, redirect the user to
        # the recipe info page of the updated recipe
...
def delete_recipe(recipe_id):
    ...
    if selected_recipe and selected_recipe["author"] == user_in_session["_id"]:
        ...
        users_coll.update_one(
            {"_id": ObjectId(user_in_session["_id"])},
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if registered_user:
            ...
            # matches the hashed password in the DB
            if check_password_hash(
                ...
                flash(
                    "Oops, something's not right in the recipe."
            ...
            flash(
                "Chop-chop, that username isn't in our kitchen!"
    ...
    return render_template(
        "login.html", form=form, title="Enter the Veggie World")
...
def register():
    ...
    return render_template("register.html",
                           form=form, title="Join the Veggie World")
...
def account_settings(username):
    if "username" not in session:
        flash("Chef, please log in to enter the"
              "Veggienosh kitchen and view that page!")
...
@app.route(
    "/change_username", methods=["GET", "POST"]
)  # Removed <username> to prevent confusion
def change_username():
    # Redirect user if they're not logged in
    if "username" not in session:
        flash("You need to be in the VeggieNosh"
              "kitchen to update your chef's name!")
        return redirect(url_for("login"))
    ...
    if form.validate_on_submit():
        ...
        if registered_user and registered_user["username"] != current_username:
            ...
            flash("Chef's name plate updated!"
                  "Next time, use your new name"
                  "to enter the Veggienosh kitchen.")
...
def change_password(username):
    if "username" not in session:
        flash("Chef, please log in to the VeggieNosh kitchen"
              "to update your secret sauce!"
              )
    users = users_coll
    ...
    if form.validate_on_submit():
        if check_password_hash(
            users.find_one({"username": username})["password"], old_password
        ):
            if new_password == confirm_password:
                ...
                flash("Recipe secret updated!"
                      "Log in with the new ingredient next time."
                      )
                return redirect(url_for("account_settings", username=username))
            else:
                flash("Oops! The new ingredients don't match. Try again.")
                return redirect(
                    url_for("change_password", username=session["username"])
                )
        else:
            flash("Oops! Old ingredient is not correct."
                  "Recheck and try again.")
            return redirect(url_for("change_password",
                            username=session["username"]))
    return render_template(
        "change_password.html", username=username,
...
def error_404(error):
    return (
        render_template("error-pages/404.html",
                        error=True, title="Page not found"),
        404,
    )
...
def error_500(error):
    return (
        render_template(
            "error-pages/500.html", error=True, title="Internal Server Error"
        ),
        500,
    )
...
def search():
    ...
    results = (
        recipes_coll.find({"recipe_name": regex_query})
        .sort("_id", pymongo.ASCENDING)
        .skip((current_page - 1) * limit_per_page)
        .limit(limit_per_page)
    )
