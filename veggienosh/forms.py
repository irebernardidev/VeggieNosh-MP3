...
class RegisterForm(FlaskForm):
    username = StringField(
        ...
        validators=[
            ...
            Length(min=3, max=15),
            Regexp(
                r"^[\S]+$", message="No whitespace allowed"
    ...
    password = PasswordField(
        ...
        validators=[
            ...
            Regexp(r"^[\S]+$", message="No whitespace allowed"),
            Regexp(
    ...
    def validate_password(self, field):
        if self.username.data.lower() == field.data.lower():
            raise ValidationError("Password cannot be"
                  "the same as your username.")
        if self.username.data and field.data.lower().startswith(
...
class ChangeUsernameForm(FlaskForm):
    ...
    new_username = StringField(
        ...
        validators=[
            ...
            Regexp(
                r"^[\S]+$", message="No whitespace allowed"
            ),  # New username must not contain whitespace
        ],
    )
    ...
    def validate_new_username(self, field):
        if self.current_username.data.lower() == field.data.lower():
            raise ValidationError(
                "New username cannot match current username.")


# Form to change the current password
class ChangePasswordForm(FlaskForm):
    old_password = PasswordField(
        "Current Password", validators=[DataRequired(), Length(min=3, max=15)]
    )
    ...
    def validate_new_password(self, field):
        if self.old_password.data.lower() == field.data.lower():
            raise ValidationError(
                ...
                "the same as the current password in any case."
            )
...
class Add_RecipeForm(FlaskForm):
    ...
    image = StringField(
        "Recipe Image", validators=[Optional()]  # Image field is optional
    )
